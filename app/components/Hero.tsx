...
const words = ["Seu", "site", "institucional,", "do", "jeito", "certo."];
  ...
  return (
    <section ref={ref} className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden">
      ...
      <motion.div style={{ y, opacity }} className="relative z-10 flex flex-col items-center text-center px-6 pt-24">
        ...
        <h1 className="text-5xl md:text-7xl lg:text-8xl font-bold leading-[1.05] max-w-5xl mb-6">
          {words.map((word, i) => (
            <motion.span
              ...
              className={`inline-block mr-4 ${i >= 3 ? "text-gradient" : ""}`}
            >
              {word}
            </motion.span>
          ))}
        ...
        >
          Um site que apresenta você com autoridade, desperta confiança no paciente
          e coloca sua marca no digital com qualidade profissional.
        </motion.p>
        ...
        >
          {["Time especializado em médicos", "Entrega em até 1 mês", "Pagamento único"].map((item) => (
